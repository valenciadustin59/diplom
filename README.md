...
- `D72` — implemented locally: early stop стал явным runtime/API contract. `/audits`, `/results` и `/events/diagnostics` отдают typed `early_stop` summary с пользовательским текстом, score band, skipped stages и статусом пропуска конкурентов; таймлайн показывает это как отдельное состояние.
