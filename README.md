...
D39 / GitHub `#58` реализован локально как model status в интерфейсе. Локальная копия плана: `plans/d39-model-status-interface.md`. Backend endpoint `GET /health/model` показывает активный runtime artifact, dataset/version metadata, guardrail metrics, publish context и rollback availability. Frontend использует эти данные только там, где они помогают объяснить работу системы, без пользовательского ML/debug clutter. Текущий runtime после D83: `active`, `QueryCoreGuardrailCatBoostRegressor`, schema `v4`, dataset `dataset-v7-final`, artifact `dataset-v7-final-20260505125858`, rollback available to D58 v5.
...
- `D58` / `#77` — completed/pushed: v5 pointwise CatBoost был опубликован как runtime artifact under competitiveness scorecard. D83 позже заменил active alias на query-core v7, а D58 v5 сохранён как rollback artifact `backend/artifacts/versions/page_quality_model--dataset-v5-20260502151507.pkl`.
...
- `D61` / `#80` — documentation pass: README, backend README, roadmap and ML appendix describe the competitiveness goal, runtime score layers and metric interpretation.
- `D62-D68` / GitHub `#81-#87` — completed retrospectively: финальный query-competitiveness contract. Runtime relevance теперь использует query relevance guardrail вокруг score, чтобы страница не по запросу не могла получить среднюю оценку только за общие SEO-сигналы. Финальный release pipeline находится в `backend/app/ml/final_query_competitiveness.py` и блокирует publish, пока `dataset-v7-final` не прошёл полный путь: collection, hard negatives, deterministic labels, split validation, training и controlled decision.
- `D69` / GitHub `#88` — completed retrospectively: сборщик `dataset-v7-final` получил domain cap (`--max-domain-rows-per-domain`, рекомендовано `12`) до parallel fetch, а hard negatives теперь материализуются из сохранённых snapshot artifacts с пересчётом признаков под новый запрос. Финальный pipeline требует `dataset.with-hard-negatives.csv`, если manifest помечает hard negatives обязательными.
- `D70` / GitHub `#89` — completed retrospectively: `query-relevance-contract-v2` разделяет relevance cap и `early_stop_decision`. Уверенный full mismatch теперь уходит в `0-5` и помечается как кандидат на раннюю остановку, а сомнительные случаи продолжают аудит в низком диапазоне (`probable_mismatch` до `25`), чтобы не отрезать релевантные страницы из-за ошибки векторизации.
- `D71` / GitHub `#90` — completed retrospectively: backend preflight теперь запускается после загрузки и базового feature extraction. Только уверенное полное несоответствие запросу останавливает аудит до heavy analysis и конкурентов; аудит завершается как `completed`, score остаётся в `0-5`, а payload содержит `score_basis=query_relevance_early_stop`.
- `D72` / GitHub `#91` — completed retrospectively: early stop стал явным runtime/API contract. `/audits`, `/results` и `/events/diagnostics` отдают typed `early_stop` summary с пользовательским текстом, score band, skipped stages и статусом пропуска конкурентов; таймлайн показывает это как отдельное состояние.
- `D73-D74` / GitHub `#92-#93` — completed retrospectively: UI показывает это как понятное состояние `Страница не соответствует запросу` в обзоре, конкурентах, истории и отчёте, без raw ML/debug полей; regression tests закрывают unrelated good pages, near-topic false positives, коммерческие модификаторы, редкие low-signal случаи и недостаточный текст.
- `D75` / GitHub `#94` — completed retrospectively as raw collection: real `dataset-v7-final` top-10 evidence now has `500/500` seed queries with successful rows, `3876` rows, `367` failures, `2253` domains, `50` categories, `5` cities and max domain repeat `12` under the cap. This is collection evidence only: `dataset.csv` still contains builder placeholder weak labels, `manifest.json` remains `ready_for_training=false`, hard negatives/labels/split are still required, and no runtime model/publish was changed.
- `D76` / GitHub `#95` — completed retrospectively: `dataset.with-hard-negatives.csv` now adds `1000` hard negatives from D75 snapshots, exactly `2` per query, covering `500/500` queries, `50/50` target categories and `50/50` source categories with `0` same-category pairs and `0` missing artifacts. Training remains blocked until deterministic expert labels and split validation are produced.
- `D77` / GitHub `#96` — completed retrospectively: `dataset.labeled.csv` materializes deterministic expert-rubric labels over all `4876` v7 rows (`3876` regular + `1000` hard negatives). Hard negatives are capped at `35.0`; `192` rows hit the cap and `0` are above it. Training is still blocked until D78 leakage-safe split validation.
- `D78` / GitHub `#97` — completed retrospectively: `split.json` now uses `group_by_query_category_stratified` over `dataset.labeled.csv`: `3898` train rows / `978` validation rows, `400/100` train/validation queries, `0` query overlap, all `50` categories and `5` non-empty cities represented in validation, hard negatives split `800/200`, and `manifest.json` is `ready_for_training=true`. No model was trained or published.
- `D79` / GitHub `#98` — completed retrospectively: trained non-production `final_query_competitiveness_catboost_v7` candidate from the D78 split. Artifact: `backend/artifacts/page_quality_model.dataset-v7-final-candidate.pkl`, SHA1 `65c30b2611e6cde393fae52d1f820a00e7d42681`, schema `v3`, `148` features, `runtime_enabled=false`. Metrics: `MAE=1.699042`, `Spearman=0.929238`, `NDCG@10=0.996686`, `top_3_hit_rate=0.91` as diagnostic. Production artifact was not changed.
- `D80` / GitHub `#99` — completed retrospectively: controlled decision on the D78 split recommends `no_publish`. The candidate beats the v5 runtime on runtime-adjusted metrics (`MAE=2.387708` vs reference `6.322645`, `Spearman=0.930727`, `NDCG@10=0.996701`), but fails `hard_negatives_learned_below_cap`: `25` validation hard negatives exceed the `35.0` cap.
- `D81` / GitHub `#100` — completed retrospectively: controlled release step recorded `no_publish` and kept production unchanged at that time. Runtime SHA1 before/after stayed `5374ca30f48f70d8629e7d84ec3df0524ef35b52`; D83 later replaced this v5 alias with query-core v7.
...
- Final demo smoke polish on `2026-05-06`: informational helper words such as `что такое`, `почему`, `какой/какая/какие`, `зачем` and `чем` are excluded from the primary query core, so informational pages are judged by the real topic word. Evidence: `output/runtime-smoke/final-demo-8case-smoke-summary.json` passed `8/8` live cases, including photosynthesis, HTML learning, PZPO crane relevance, winemore/litres/labirint mismatches and a 404/unusable page.
- Query parsing dictionaries live in `backend/app/query_lexicons.py`. They only describe bounded helper/intent/geo classes and aliases; the project does not maintain a topic/core dictionary. This keeps relevance scoring open-ended while still handling common city variants such as `спб`, `питер`, `санкт-петербург`, `мск` and large Russian city names.
