...
- `D75` — started locally: real `dataset-v7-final` top-10 collection began with domain cap `12`. Current controlled batch has `10/500` queries, `80` successful rows, `6` failures, `64` domains, `80` snapshot artifacts and `0` domain-cap skips. This is collection evidence only: `dataset.csv` still contains builder placeholder weak labels, `manifest.json` remains `ready_for_training=false`, and no runtime model/publish was changed.
