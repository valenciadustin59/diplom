...
- `D81` / GitHub `#100` — completed retrospectively: controlled release step recorded `no_publish` and kept production unchanged. Runtime SHA1 before/after stayed `5374ca30f48f70d8629e7d84ec3df0524ef35b52`; active runtime remains `dataset-v5`.
- `D82` — completed locally: query-core hardening fixes the D80 hard-negative blocker without publishing automatically. New non-production candidate: `backend/artifacts/page_quality_model.dataset-v7-final-query-core-candidate.pkl`, SHA1 `2bbf84bfcc77662d66f62cfdafbb6ee4d8264e88`, schema `v4`, `156` features. D82 result: validation hard negatives above `35.0` are now `0` instead of `25`; runtime-adjusted metrics beat the current v5 reference (`MAE=4.253847` vs `6.556754`, `Spearman=0.917044` vs `0.77064`, `NDCG@10=0.996873` vs `0.984659`). Decision evidence says `publish_candidate`, but runtime remains `dataset-v5` until a separate controlled publish is explicitly run.
- `D83` — completed locally: query-core publish readiness. The old `--publish` path still belongs to D80/D81 v3, so D83 adds explicit readiness and publish commands for the D82 v4 candidate. Run `backend\.venv\Scripts\python.exe -m app.ml.final_query_competitiveness --prepare-query-core-publish` to verify deployment preconditions; current readiness is green with failed checks `[]`. Run `--publish-query-core` only when the user explicitly chooses to replace runtime.
