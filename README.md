...
D37 / GitHub `#56` реализован локально как unified `v3` feature model with hybrid/top-3 guardrail evidence. Локальная копия плана: `plans/d37-unified-v3-hybrid-ensemble-top3-guardrail.md`. D37 добавил `MODEL_SCHEMA_VERSION_V3` на `148` pre-competitor features (`59` baseline + `49` technical/commercial + `25` heavy-analysis + `15` intent-alignment), собрал `backend/data/dataset_versions/dataset-v3-d37/` на `885` строк и `99` запросов, обучил non-production RF/CatBoost/CatBoostRanker candidates и сравнил их с текущим reference. Shadow benchmark `backend/artifacts/ranking-benchmarks/dataset-v3-d37-shadow/shadow-benchmark-guardrails-report.json` рекомендует `publish_candidate` и выбирает `pointwise_catboost`: `top_3_hit_rate=0.95`, `ndcg_at_10=0.945929`, `spearman_mean=0.421894`, `MAE=11.774165` против reference `0.95`, `0.909302`, `0.153604`, `23.858757`. `backend/artifacts/page_quality_model.pkl` пока не заменён; SHA1 остался `5600b5f3fff9b1b7590bc90b2b5fbc24ec5466f9`. Следующий практический шаг — controlled publish/smoke для CatBoost v3 candidate, если нужен rollout в продукт.
