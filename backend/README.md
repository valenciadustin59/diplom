...
- `D80` / GitHub `#99` runs the controlled product decision for the D79 candidate on the D78 split. Decision is `no_publish`: runtime-adjusted candidate metrics are stronger than the current v5 runtime (`MAE=2.387708` vs reference `6.322645`), but `hard_negatives_learned_below_cap` fails because `25` validation hard negatives exceed the `35.0` cap.
- `D81` / GitHub `#100` records controlled no-publish evidence. Production alias `artifacts/page_quality_model.pkl` remains unchanged at SHA1 `5374ca30f48f70d8629e7d84ec3df0524ef35b52`; runtime still uses `dataset-v5`.
- `D82` completes query-core hardening for the final v7 candidate without changing the production alias. It adds schema `v4` with `156` features, materializes `data/dataset_versions/dataset-v7-final/dataset.query-core.csv`, trains `artifacts/page_quality_model.dataset-v7-final-query-core-candidate.pkl` (SHA1 `2bbf84bfcc77662d66f62cfdafbb6ee4d8264e88`) and records controlled decision evidence in `artifacts/ranking-benchmarks/dataset-v7-final-d82/`. D82 fixes the D80 blocker: validation hard negatives above `35.0` are now `0` instead of `25`; runtime-adjusted metrics are `MAE=4.253847`, `Spearman=0.917044`, `NDCG@10=0.996873`. Decision is `publish_candidate`, but runtime remains `dataset-v5` until a separate controlled publish is explicitly run.
- `D83` adds deployment readiness for D82. `--prepare-query-core-publish` checks that the D82 decision is publishable, the candidate loads, schema is `v4`, feature count is `156`, hard-negative guardrails passed, production still matches the D82 reference SHA and production has not already been replaced. Current readiness report is green: `artifacts/ranking-benchmarks/dataset-v7-final-d83/d83-query-core-publish-readiness-report.json`. Use `--publish-query-core` only after an explicit user decision to switch runtime.
