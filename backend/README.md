...
- `D62-D68` add the final query-competitiveness contract. Query relevance is applied around the runtime model score, so pages unrelated to the query cannot stay in a medium range because of good generic SEO signals. `app.ml.final_query_competitiveness` blocks training/publish until `dataset-v7-final` has collection, hard negatives, deterministic labels, split validation and controlled decision evidence.
...
- `D77` materializes deterministic expert-rubric labels into `dataset.labeled.csv` from `dataset.with-hard-negatives.csv`. The labeled surface has `4876` rows, `1000` hard negatives, hard-negative score cap `35.0`, `192` cap applications and `0` hard negatives above cap. Training still waits for D78 split validation.
- `D78` materializes leakage-safe split evidence. `split.json` is `group_by_query_category_stratified` with `3898` train rows, `978` validation rows, `400/100` train/validation queries, `0` query overlap, all `50` categories and all `5` non-empty cities represented in validation, hard negatives split `800/200`, and `manifest.json` now reports `ready_for_training=true`.
- `D79` trains the non-production final candidate `page_quality_model.dataset-v7-final-candidate.pkl`. It is `CatBoostRegressor`, schema `v3`, `148` features, SHA1 `65c30b2611e6cde393fae52d1f820a00e7d42681`, `non_production=true`, `runtime_enabled=false`. Validation metrics: `MAE=1.699042`, `Spearman=0.929238`, `NDCG@10=0.996686`, `top_3_hit_rate=0.91` diagnostic. Runtime alias `artifacts/page_quality_model.pkl` remains unchanged.
- `D80` runs the controlled product decision for the D79 candidate on the D78 split. Decision is `no_publish`: runtime-adjusted candidate metrics are stronger than the current v5 runtime (`MAE=2.387708` vs reference `6.322645`), but `hard_negatives_learned_below_cap` fails because `25` validation hard negatives exceed the `35.0` cap.
- `D81` records controlled no-publish evidence. Production alias `artifacts/page_quality_model.pkl` remains unchanged at SHA1 `5374ca30f48f70d8629e7d84ec3df0524ef35b52`; runtime still uses `dataset-v5`.
