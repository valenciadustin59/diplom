...
D39 / GitHub `#58` реализован локально как model status in interface. Backend endpoint `GET /health/model` отдаёт активный artifact, SHA1, metadata sidecar, model/dataset sections, `metrics_summary`, publish context and rollback reference. После D83 endpoint describes the active `dataset-v7-final` query-core runtime artifact. План: `../plans/d39-model-status-interface.md`.
...
- `D58` published `pointwise_catboost_v5` under the competitiveness scorecard. D83 later superseded this alias with the query-core v7 runtime; the D58 v5 artifact is preserved as rollback evidence in `artifacts/versions/page_quality_model--dataset-v5-20260502151507.pkl`.
...
- `D62-D68` / GitHub `#81-#87` add the final query-competitiveness contract. Query relevance is applied around the runtime model score, so pages unrelated to the query cannot stay in a medium range because of good generic SEO signals. `app.ml.final_query_competitiveness` blocks training/publish until `dataset-v7-final` has collection, hard negatives, deterministic labels, split validation and controlled decision evidence.
- `D69` / GitHub `#88` prepares the live `dataset-v7-final` collection path: `app.ml.dataset_builder` supports `--max-domain-rows-per-domain` and records skipped rows in dataset metadata; `app.ml.final_hard_negatives` writes `dataset.with-hard-negatives.csv` by recomputing features from saved snapshots under different target queries. The final training pipeline now requires that hard-negative file when the v7 manifest says hard negatives are mandatory.
- `D70` / GitHub `#89` introduces `query-relevance-contract-v2` and `query-relevance-early-stop-v1`. Confident full mismatch is now capped to `0-5` and marked as a future early-stop candidate; ambiguous mismatch continues the audit as `probable_mismatch` up to `25` so noisy vectorization does not prematurely stop rare but relevant pages.
- `D71` / GitHub `#90` wires that preflight into audit orchestration. Target fetch now goes through feature extraction before heavy analysis; confident full mismatch finishes as a completed early-stop audit with `competitor_processing_status=skipped_early_stop`, no heavy-analysis/competitor fan-out and `comparison_summary.score_basis=query_relevance_early_stop`.
- `D72` / GitHub `#91` makes early stop a stable runtime/API contract. Audit status, list, results and timeline diagnostics expose typed `early_stop` summary with score band, user-facing copy, skipped stages and competitor skip status.
- `D73-D74` / GitHub `#92-#93` add product UI/copy and regression coverage around this contract. The frontend shows `Страница не соответствует запросу` instead of raw ML/debug fields, while backend tests cover unrelated good pages, near-topic false positives, commercial modifiers, rare low-signal pages and insufficient extraction.
- `D75` / GitHub `#94` completed live raw `dataset-v7-final` collection. Final raw coverage: `500/500` seed queries with successful rows, `3876` rows, `367` failures, `2253` unique domains, `50` categories, `5` cities and max single-domain repeat `12` under cap `12`. This is not training-ready: builder weak labels in `dataset.csv` are placeholders, deterministic expert labels and hard negatives are still required.
- `D76` / GitHub `#95` materializes `dataset.with-hard-negatives.csv` from D75 snapshots. It adds `1000` hard negatives, `2` per query, with all `500` queries, all `50` target/source categories, `0` same-category pairs and `0` missing artifacts. Validation now passes hard-negative checks and blocks only on the intentional `manifest_ready_for_training=false`.
- `D77` / GitHub `#96` materializes deterministic expert-rubric labels into `dataset.labeled.csv` from `dataset.with-hard-negatives.csv`. The labeled surface has `4876` rows, `1000` hard negatives, hard-negative score cap `35.0`, `192` cap applications and `0` hard negatives above cap. Training still waits for D78 split validation.
- `D78` / GitHub `#97` materializes leakage-safe split evidence. `split.json` is `group_by_query_category_stratified` with `3898` train rows, `978` validation rows, `400/100` train/validation queries, `0` query overlap, all `50` categories and all `5` non-empty cities represented in validation, hard negatives split `800/200`, and `manifest.json` now reports `ready_for_training=true`.
- `D79` / GitHub `#98` trains the non-production final candidate `page_quality_model.dataset-v7-final-candidate.pkl`. It is `CatBoostRegressor`, schema `v3`, `148` features, SHA1 `65c30b2611e6cde393fae52d1f820a00e7d42681`, `non_production=true`, `runtime_enabled=false`. Validation metrics: `MAE=1.699042`, `Spearman=0.929238`, `NDCG@10=0.996686`, `top_3_hit_rate=0.91` diagnostic. Runtime alias `artifacts/page_quality_model.pkl` remains unchanged.
- `D80` / GitHub `#99` runs the controlled product decision for the D79 candidate on the D78 split. Decision is `no_publish`: runtime-adjusted candidate metrics are stronger than the then-active D58 v5 runtime (`MAE=2.387708` vs reference `6.322645`), but `hard_negatives_learned_below_cap` fails because `25` validation hard negatives exceed the `35.0` cap.
- `D81` / GitHub `#100` records controlled no-publish evidence. Production alias `artifacts/page_quality_model.pkl` remained unchanged at SHA1 `5374ca30f48f70d8629e7d84ec3df0524ef35b52`; D83 later replaced it through the explicit query-core publish path.
...
- Final demo smoke polish on `2026-05-06` tightened query-core extraction for informational queries: helper words such as `что`, `такое`, `почему`, `какой/какая/какие`, `зачем` and `чем` are modifiers, not the main topic. Evidence: `../output/runtime-smoke/final-demo-8case-smoke-summary.json` passed `8/8` live cases; targeted backend verification for features/query relevance/final competitiveness passed with `53 passed`.
- Query parsing lexicons are centralized in `app.query_lexicons`: helper words, intent modifiers, geo aliases and token-alias groups. These dictionaries are used only to remove/interpret bounded modifiers; topic words are not hardcoded and continue to be evaluated by lexical/NLP signals.
