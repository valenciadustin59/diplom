...
def test_dataset_v7_manifest_preserves_seed_policy_after_controlled_publish() -> None:
    ...
    assert dataset_manifest["status"] == "runtime_published"
    ...
    assert dataset_manifest["release_progress"]["decision"] == "published"
    assert dataset_manifest["release_progress"]["runtime_enabled"] is True
    assert dataset_manifest["quality_gates"]["controlled_release_decision_recorded"] is True
    assert dataset_manifest["quality_gates"]["production_changed_only_for_controlled_publish"] is True
