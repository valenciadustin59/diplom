...
- `dataset-v7-final` now has raw top-10 collection, hard negatives, deterministic labels, leakage-safe split validation, non-production candidate training and controlled D80/D81 release evidence.
...
- Running `cd backend && .venv\Scripts\python.exe -m app.ml.final_query_competitiveness --decide` writes D80 controlled decision evidence; `--publish` writes D81 controlled release/no-publish evidence.
