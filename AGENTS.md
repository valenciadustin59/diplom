...
- `D32` / GitHub `#51` - completed locally: `197` expert-rubric labels added for `dataset-v2`; D33 must refresh dataset rows/manifest/split to apply them.
...
- D33 evidence: `backend/data/dataset_versions/dataset-v2/dataset.csv` now has `197` `hybrid` rows and `688` `weak_serp` rows; `manifest.json` is `ready_for_training=true`; `split.json` is `group_by_query` with no query overlap. D34 is next.
