...
- `plans/d82-query-core-hard-negative-hardening.md`
- `plans/d83-query-core-publish-readiness.md`
...
- `D76`: completed locally; `dataset.with-hard-negatives.csv` has `4876` rows with `1000` hard negatives, `2` per query, all `50` target/source categories, no same-category pairs and no missing artifacts. No training or publish yet.
- `D77`: completed locally; `dataset.labeled.csv` has `4876` deterministic expert-rubric labels, `1000` hard negatives, hard-negative cap `35.0`, `192` cap applications and `0` hard negatives above cap. No training or publish yet.
- `D78`: completed locally; `split.json` has `3898/978` train/validation rows, `400/100` train/validation queries, `0` query overlap, all `50` categories represented in validation, hard negatives split `800/200`, and `manifest.json` is `ready_for_training=true`. No training or publish yet.
- `D79`: completed locally; non-production `final_query_competitiveness_catboost_v7` artifact trained at `backend/artifacts/page_quality_model.dataset-v7-final-candidate.pkl`, SHA1 `65c30b2611e6cde393fae52d1f820a00e7d42681`, schema `v3`, `148` features, `runtime_enabled=false`. No publish yet.
