...
- `D75`: started locally; controlled `dataset-v7-final` collection has `10/500` seed queries, `80` successful rows, `6` failures, `64` domains and `80` saved snapshot artifacts. No training or publish yet.
...
Active post-publish operations tasks: continue `dataset-v7-final` collection from `--seed-offset 10` with `--max-domain-rows-per-domain 12`, materialize hard negatives, then label/train/decide before any controlled publish.
