...
Active post-publish operations tasks: collect `dataset-v7-final` with `--max-domain-rows-per-domain 12`, materialize hard negatives, then label/train/decide before any controlled publish. D72 remains unselected/untouched unless the next issue explicitly defines it.
